...
import { addInterviewMessage, createInterview, finishInterview, getInterview, listInterviewMessages } from "../../modules/interview/interviewRepo.js";
...
apiMeInterviewRouter.post("/start", async (req, res, next) => {
    try {
        ...
        return res.status(201).json(sanitizeDeep({
            ok: true,
            interview,
...
apiMeInterviewRouter.post("/:interviewId/message", async (req, res, next) => {
    try {
        ...
        if (interview.status !== "active")
            return res.status(409).json({ ok: false, error: "INTERVIEW_NOT_ACTIVE" });
        const user = await getOrCreateUserProfile(db, req.user.uid, req.user.email ?? null);
