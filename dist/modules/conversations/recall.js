...
import { listOwnerConversationsByAssistant } from "./conversationsRepo.js";
function clip(s, max) {
    const t = (s ?? "").trim();
    if (t.length <= max)
        return t;
...
export async function buildAssistantRecallContext(opts) {
    ...
    const convos = await listOwnerConversationsByAssistant(opts.db, opts.ownerUid, opts.assistantId, Math.max(10, opts.maxConversations ?? 12));
