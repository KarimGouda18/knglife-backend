...
function clampDurationSeconds(s) {
...
function requiredExtensionHops(targetSeconds) {
    if (targetSeconds <= 8)
        return 0;
    return Math.min(20, Math.ceil((targetSeconds - 8) / 7));
}
function continuationPrompt(basePrompt, hopIndex, totalHops) {
 ...
 */
async function concatenateVideos(videoBuffers) {
...
export async function generateConversationVideo(opts) {
    ...
    const baseVideoBytes = typeof currentVideoRef?.videoBytes === "string" && currentVideoRef.videoBytes.length > 0
    ...
    for (let i = 1; i <= hops; i++) {
        ...
        const extVideoBytes = typeof currentVideoRef?.videoBytes === "string" && currentVideoRef.videoBytes.length > 0
            ? Buffer.from(currentVideoRef.videoBytes, "base64")
