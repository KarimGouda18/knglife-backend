...
import { addMessage, createConversation, getConversation, updateConversation, newId } from "../modules/conversations/conversationsRepo.js";
...
function normalizePrivateWsError(err) {
...
function pickVoiceName(assistant) {
    return assistant?.voiceName || "Kore";
}
function extractSignals(m) {
    const outTr = m?.serverContent?.outputTranscription?.text ??
        m?.outputAudioTranscription?.text ??
        m?.outputTranscription?.text ??
        null;
    const inTr = m?.serverContent?.inputTranscription?.text ??
    ...
    const result = {};
    if (audioData)
        result.outputAudio = { dataBase64: audioData, mimeType: audioMime ?? "audio/pcm;rate=24000" };
    if (typeof outTr === "string" && outTr.trim())
        result.outputTranscription = outTr.trim();
    if (typeof inTr === "string" && inTr.trim())
        result.inputTranscription = inTr.trim();
    return result;
}
...
class TranscriptSaver {
    ...
    MIN_LEN = 8;
    MAX_BUF = 2000;
    constructor(db, uid, interviewId) {
        this.db = db;
        this.uid = uid;
        this.interviewId = interviewId;
    }
    mergeIncremental(prev, next) {
    ...
    scheduleFlush(role) {
        ...
        if (role === "user")
            ...
            this.assistantTimer = timer;
    }
    appendUserChunk(chunk) {
        const c = normalizeSpaces(chunk);
        if (!c)
            return;
        this.userBuf = this.mergeIncremental(this.userBuf, c);
        if (this.userBuf.length >= this.MAX_BUF)
            void this.flush("user");
        else
            this.scheduleFlush("user");
    }
    appendAssistantChunk(chunk) {
        ...
        if (this.assistantBuf.length >= this.MAX_BUF)
            void this.flush("assistant");
        else
            this.scheduleFlush("assistant");
    }
    async flush(role) {
        if (role === "user") {
            if (this.userTimer)
...
async function sendRealtimeVideoFrame(session, opts) {
...
export function attachLiveWebSocketServer(server) {
    ...
    wss.on("connection", async (ws, req) => {
        ...
        const bumpVad = async (isInterview, session) => {
            if (isInterview)
                return;
            if (vadTimer)
