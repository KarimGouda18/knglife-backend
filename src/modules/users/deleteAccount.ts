// src/modules/users/deleteAccount.ts
 ...
 */
async function deleteQueryInBatches(db: Firestore, query: FirebaseFirestore.Query, batchSize = 400) {
 ...
 */
async function deleteSubcollection(db: Firestore, docRef: FirebaseFirestore.DocumentReference, subName: string) {
 ...
 */
async function deleteStoragePrefix(bucket: Bucket, prefix: string) {
  try {
...
export async function deleteAccountEverywhere(opts: {
  ...
  ok: true;
  deleted: {
    assistants: number; // stima (conteggio docs letti prima di delete)
